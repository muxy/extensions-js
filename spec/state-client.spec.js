...
describe('StateClient', () => {
  ...
  it('should fail with invalid JWT', () => {
    ...
    return expect(client.getRank()).rejects.toBeDefined();
  });
  ...
  it('should make a request', function() {
    server.respondWith('GET', 'https://api.muxy.io/v1/e/rank?id=default', rankOK({ data: [] }));
    ...
    server.respond();
    return expect(out).resolves.toHaveProperty('data');
  });

  /** @test {StateClient#getRank} */
  it('should fail on invalid json', function() {
    server.respondWith('GET', 'https://api.muxy.io/v1/e/rank?id=default', [
      200,
      {
    ...
    const out = client.getRank();
    server.respond();

    return expect(out).rejects.toBeDefined();
  });
});
